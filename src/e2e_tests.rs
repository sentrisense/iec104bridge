...
async fn float_message_dispatches_as_measured_float() {
    let calls = run(
        vec![parse(r#"{"ioa": 100, "value": 42.5, "type": "float"}"#)],
        1,
    )
    .await;
...
async fn scaled_message_dispatches_as_measured_scaled() {
    let calls = run(
        vec![parse(r#"{"ioa": 200, "value": 1000, "type": "scaled"}"#)],
        1,
    )
    .await;
    assert_eq!(calls.len(), 1);
...
async fn normalized_message_dispatches_as_measured_float() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 300, "value": 0.75, "type": "normalized"}"#,
        )],
        1,
    )
    .await;
...
async fn single_point_true_dispatches_as_on() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 400, "value": true, "type": "single_point"}"#,
        )],
        1,
    )
    .await;
...
async fn single_point_false_dispatches_as_off() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 401, "value": false, "type": "single_point"}"#,
        )],
        1,
    )
    .await;
...
async fn double_point_falls_back_to_single_point() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 500, "value": true, "type": "double_point"}"#,
        )],
        1,
    )
    .await;
...
async fn missing_ca_falls_back_to_default() {
    let calls = run(vec![parse(r#"{"ioa": 1, "value": true}"#)], 7).await;
    assert_eq!(calls.len(), 1);
...
async fn good_quality_propagated() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 1, "value": 0.0, "type": "float", "quality": "good"}"#,
        )],
        1,
    )
    .await;
    assert!(
        matches!(&calls[0], SentCall::MeasuredFloat { quality, .. } if *quality == Quality::GOOD)
    );
}
...
async fn invalid_quality_propagated() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 1, "value": 0.0, "type": "float", "quality": "invalid"}"#,
        )],
        1,
    )
    .await;
    assert!(
        matches!(&calls[0], SentCall::MeasuredFloat { quality, .. } if *quality == Quality::INVALID)
    );
}
...
async fn not_topical_quality_propagated() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 1, "value": 0.0, "type": "float", "quality": "not_topical"}"#,
        )],
        1,
    )
    .await;
    assert!(
        matches!(&calls[0], SentCall::MeasuredFloat { quality, .. } if *quality == Quality::NOT_TOPICAL)
    );
}
...
async fn substituted_quality_propagated() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 1, "value": 0.0, "type": "float", "quality": "substituted"}"#,
        )],
        1,
    )
    .await;
    assert!(
        matches!(&calls[0], SentCall::MeasuredFloat { quality, .. } if *quality == Quality::SUBSTITUTED)
    );
}
...
async fn default_quality_is_good_when_absent() {
    let calls = run(
        vec![parse(r#"{"ioa": 1, "value": 0.0, "type": "float"}"#)],
        1,
    )
    .await;
    assert!(
...
async fn periodic_cot_propagated() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 1, "value": 1.0, "type": "float", "cot": "periodic"}"#,
        )],
        1,
    )
    .await;
...
async fn background_scan_cot_propagated() {
    let calls = run(
        vec![parse(
            r#"{"ioa": 1, "value": 1.0, "type": "float", "cot": "background_scan"}"#,
        )],
        1,
    )
    .await;
    assert!(matches!(
        &calls[0],
        SentCall::MeasuredFloat {
            cot: CauseOfTransmission::Background,
            ..
        }
    ));
}
...
async fn mixed_ca_messages_resolved_independently() {
    ...
    assert!(matches!(
        &calls[0],
        SentCall::MeasuredFloat { ioa: 10, ca: 1, .. }
    ));
    assert!(matches!(
        &calls[1],
        SentCall::MeasuredFloat { ioa: 20, ca: 9, .. }
    ));
    assert!(matches!(
        &calls[2],
        SentCall::MeasuredFloat { ioa: 30, ca: 5, .. }
    ));
}
